...
export function AnamnesisDetailsView({ record }: AnamnesisDetailsViewProps) {
    const socialDataKeys: (keyof AnamnesisFormValues)[] = ['nome_cliente', 'data_nascimento', 'telefone', 'email', 'profissao', 'estado_civil', 'nivel_atividade', 'suporte_social', 'compreensao_adesao'];
  ...
  return (
    <div className="space-y-4 p-4">
        <Section title="S - Social / Fatores do Paciente">
            {socialDataKeys.map(key => <DetailItem key={key} label={key} value={record[key]} />)}
        </Section>
        <Section title="Dados Clínicos e Comorbidades">
